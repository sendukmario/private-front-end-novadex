...
const API_DC_REST_URL = process.env.NEXT_PUBLIC_REST_DISCORD_MONITOR_URL;

export const fetchTokenPings = async (address: string) => {
  try {
    const { data } = await axios.get<PingResponseDiscord[]>(
      `${API_DC_REST_URL}/pinged-tokens?token=${address}&licenseKey=dev-test`,
    );
    return data[0];
  } catch (err) {
...
export const updateDiscordMonitorChannel = async (props: string[]): Promise<UpdateDiscordMonitorChannelResponse> => {
