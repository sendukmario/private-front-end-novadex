...
>(({ className, inset, ...props }, ref) => (
  <ContextMenuPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 dark:focus:bg-neutral-800 dark:focus:text-neutral-50",
      inset && "pl-8",
      className,
    )}
    {...props}
...
class ContextMenuErrorBoundary extends React.Component<
...
> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.warn(
      "React #185 Error Related 🚫 - ContextMenuContent crashed:",
      error,
      errorInfo,
    );
    Sentry.captureException(
      new Error(
        `Error 🔴 - (ContextMenuContent crashed:): ${errorInfo} | ${error}`,
      ),
    );
  }
