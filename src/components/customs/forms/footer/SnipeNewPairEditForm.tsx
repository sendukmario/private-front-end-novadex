...
export default function SnipeNewPairEditForm({ toggleModal, prevData }: Props) {
  ...
  const form = useForm<EditNewPairTaskRequest>({
    resolver: zodResolver(editNewPairTaskSchema),
    defaultValues: {
  ...
  const mutation = useMutation({
    mutationFn: editNewPairTask,
    onSuccess: () => {
      toast.custom((t: any) => (
        <CustomToast
          tVisibleState={t.visible}
          message="Edit sniper new pair successfully"
          state="SUCCESS"
        />
      ));
  ...
  useEffect(() => {
    if (prevData) {
      form.reset({
        ...
        taskId: prevData?.taskId,
        mint: prevData?.mint,
        amount: Number(prevData?.amount),
        slippage: Number(prevData?.slippage),
        autoTipEnabled: prevData?.autoTipEnabled,
        fee: Number(prevData?.fee),
        minTip: Number(prevData?.minTip),
        maxTip: Number(prevData?.maxTip),
        developer: prevData?.developer || "",
        ticker: prevData?.ticker || "",
        mevProtectEnabled: prevData?.processor == "secure" ? true : false,
        minAmountOut: Number(prevData?.minAmountOut),
        mode: ["secure", "fast"].includes(prevData?.processor)
          ? (prevData?.processor as "secure" | "fast")
          : "secure",
        wallet: prevData?.wallet,
      });
  ...
  // Save Configuration 🆙
  const onSubmit = (data: EditNewPairTaskRequest) => {
    mutation.mutate({
      ...data,
  ...
  return (
    <OverlayScrollbarsComponent
      defer
      element="div"
      className="modal__overlayscrollbar relative h-[520px] w-full flex-grow overflow-y-scroll 2xl:h-auto"
    >
      <Form {...form}>
        <form
          onSubmit={(e) => {
          ...
          <div className="flex w-full flex-col gap-y-2 p-4">
            ...
            >
              <span className="inline-block text-nowrap font-geistSemiBold text-base text-[#080811]">
                {mutation.isPending
                  ? "Processing..."
                  : `Save Snipe ${lastFocusOn == "picker" ? picker : form.watch("amount") || "0"} SOL`}
              </span>
            </BaseButton>
          </div>
        </form>
