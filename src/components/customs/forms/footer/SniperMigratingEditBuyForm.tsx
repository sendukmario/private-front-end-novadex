...
export default function SnipeMigrationBuyForm({
  form,
  toggleModal,
  prevData,
}: Props) {
  ...
  useEffect(() => {
    if (prevData) {
      ...
      form.setValue("method", "buy");
      form.setValue("amount", Number(prevData?.amount));
      ...
      form.setValue("autoTipEnabled", prevData?.autoTipEnabled || false);
  ...
  const onSubmit = (data: EditMigrationTaskRequest) => {
    mutation.mutate({
      ...data,
      amount: lastFocusOn == "picker" ? picker : data.amount,
